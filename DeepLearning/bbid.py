"""
This Python code is 99.9% the same as the Bulk-Bing-Image-downloader (https://github.com/ostrolucky/Bulk-Bing-Image-downloader). 
The only difference is that I added another command line argument -p --pages to allow the search result scraping to start 
from the specified images. This helps when you want to crawl the bing image search results for the same search keyword multiple times,
but every time it starts from different images. In this way, you can enrich your image set by crawling it multiple times. A larger image set
can significantly improve your model accuracy.
"""

#!/usr/bin/env python3
import os, urllib.request, re, threading, posixpath, urllib.parse, argparse, atexit, random, socket, time, hashlib, pickle, signal
...
if __name__ == "__main__":
	...
	parser.add_argument('-p', '--pages', help = 'Start index', required = False)
